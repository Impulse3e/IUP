...
@dataclass
class ProctorConfig:
    yaw_threshold: float = 38.0
    pitch_threshold: float = 55.0
    roll_threshold: float = 22.0
    confirm_frames: int = 24
    focus_confirm_frames: int = 36
    identity_confirm_frames: int = 8
    alert_cooldown: float = 45.0
    reminder_interval: float = 120.0
    audio_quiet_rms: float = 0.0
    audio_loud_rms: float = 480.0
    audio_sustain_sec: float = 4.0
...
class ProctoringEngine:
    ...
    def _track_streak(
        ...
        state = self.violations[violation]
        needed = confirm if confirm is not None else self.config.confirm_frames
        if condition:
            state.clear_streak = 0
            state.streak += 1
            if state.streak >= needed and not state.active:
                state.active = True
                self._raise(violation, alert_message, now, severity=severity)
            elif state.active and reminder_message and self._can_alert(violation, now, is_reminder=True):
                self._raise(violation, reminder_message, now, is_reminder=True, severity=severity)
        else:
            state.streak = 0
            if state.active:
                state.clear_streak += 1
                if state.clear_streak >= needed:
    ...
    def analyze_audio(self, rms: float, now: float) -> None:
        ...
        quiet_cut = self.config.audio_quiet_rms
        if quiet_cut > 0 and rms < quiet_cut:
    ...
    def analyze_focus(self, lost: bool, now: float) -> None:
        self._track_streak(
            ViolationType.WINDOW_FOCUS_LOST,
            lost,
            now,
            "окно экзамена потеряло фокус.",
            "фокус по-прежнему не на экзамене.",
            severity="high",
            confirm=self.config.focus_confirm_frames,
        )

    def analyze_identity(self, mismatch: bool, now: float, message: str) -> None:
        self._track_streak(
            ViolationType.IDENTITY_MISMATCH,
            mismatch,
            now,
            message,
            "лицо по-прежнему не совпадает с эталоном.",
            severity="critical",
